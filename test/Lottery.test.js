...
beforeEach( async () => { //Before we run each test it resets the accounts
...
describe('Lottery Contract', () => {
    it('deployd a contract', () => {
        assert.ok(lottery.options.address);
    });

    it('allows one account to enter', async () => {
        await lottery.methods.enter().send({ // Enter the lottery
            from: accounts[0],
    ...
    it('sends money to the winner and resets player array', async () => {
        ...
        // check that player array is empty
        const players = await lottery.methods.getPlayers().call({
            from: accounts[0]
        });
